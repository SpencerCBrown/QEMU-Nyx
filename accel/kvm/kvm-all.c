...
/*
    For now, just letting Nyx continue setting things up for dirty ring as before.
    QEMU seems to have slightly different ideas about how to use it.
    Ex: why does QEMU say KVM_CAP_MANUAL_DIRTY_LOG_PROTECT2 is incompatible with dirty ring, but Nyx enables it?
    TODO(Spencer): Revisit this
*/
#ifdef QEMU_NYX
    if (s->nyx_dirty_ring) {
