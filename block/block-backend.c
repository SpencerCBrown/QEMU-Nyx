...
#else
// TODO(Spencer): May need to remove this static at least, unsure about GRAPH_RDLOCK
static int coroutine_fn GRAPH_RDLOCK
blk_check_byte_request(BlockBackend *blk, int64_t offset, int64_t bytes)
#endif
{
...
#else
void coroutine_fn blk_aio_write_entry(void *opaque)
#endif
{
    BlkAioEmAIOCB *acb = opaque;
    BlkRwCo *rwco = &acb->rwco;
...
#else
    // TODO(Spencer): May need to add assert here
